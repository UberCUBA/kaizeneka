...
class _EditHabitPageState extends State<EditHabitPage> {
  ...
  void initState() {
    super.initState();
    // Pre-fill with current habit data
    _titleController.text = widget.habit.title;
    _notesController.text = widget.habit.notes ?? '';
    _difficulty = widget.habit.difficulty;
    _repeatType = widget.habit.repeatType;
    _icon = widget.habit.icon;
    _goalType = widget.habit.goalType;
    _targetDurationController.text = widget.habit.targetDuration?.toString() ?? '';
    _targetRepetitionsController.text = widget.habit.targetRepetitions?.toString() ?? '';
    _selectedDays = List.from(widget.habit.selectedDays);

    // Pre-fill nuevos campos
    _reminderEnabled = widget.habit.reminderEnabled;
    _reminderTime = widget.habit.reminderTime;
    _reminderDescriptionController.text = widget.habit.reminderDescription ?? '';
    _endType = widget.habit.endType;
    _endDate = widget.habit.endDate;
    _endDaysController.text = widget.habit.endDays?.toString() ?? '';
  }
  ...
  Future<void> _saveHabit() async {
    ...
    try {
      ...
      await habitProvider.updateHabit(widget.habit.id, updatedHabit);
  ...
  Future<void> _deleteHabit() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('¿Estás seguro de que quieres eliminar este hábito? Esta acción no se puede deshacer.'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        if (mounted) {
          Navigator.of(context).pushNamedAndRemoveUntil('/tasks', (route) => false, arguments: 0); // Close edit page and go to habits tab
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al eliminar hábito: $e'),
              backgroundColor: Colors.red,
            ),
          );
        }
